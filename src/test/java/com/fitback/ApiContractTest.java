...
/**
 * Full-stack contract test: exercises the real JPA-backed auth stack (domain.auth)
 * together with the core /api/v1 surface, against the local Postgres/Redis instance
 * started by scripts/start-local.ps1. Each test uses a unique email per run since
 * tenant data now persists across runs (see JpaTenantDataRepository).
 */
...
class ApiContractTest {
    ...
    void mapsEverySpecifiedApiRoute() {
        ...
        Set<String> expected = Set.of(
                ...
                "POST /api/v1/auth/logout", "GET /api/v1/store", "POST /api/v1/store",
    ...
    void supportsCoreCustomerWorkflow() throws Exception {
        String token = registerAndLogin(uniqueEmail("owner"));
    ...
    void supportsDrawioConsultationPreviewSaveAndPollingWorkflow() throws Exception {
        String token = registerAndLogin(uniqueEmail("drawio"));
    ...
    @Test
    void isolatesCustomerDataPerAuthenticatedUser() throws Exception {
        String first = registerAndLogin(uniqueEmail("first"));
        String second = registerAndLogin(uniqueEmail("second"));
    ...
    private String uniqueEmail(String label) {
        return label + "-" + UUID.randomUUID() + "@fitback.test";
    }

    private void registerOnly(String email) throws Exception {
        mockMvc.perform(post("/api/v1/auth/register").contentType(MediaType.APPLICATION_JSON)
                .content("{\"email\":\"" + email + "\",\"password\":\"secret123\",\"passwordConfirm\":\"secret123\","
                        + "\"nickname\":\"" + email.substring(0, Math.min(20, email.indexOf('@'))) + "\",\"agreeTerms\":true}"));
    }
