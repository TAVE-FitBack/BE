...
public class InquiryController {
    ...
    @ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "400",
                    description = "이미 상담으로 전환된 문의"
            ),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "404",
                    description = "문의가 없거나 현재 매장 소속이 아님"
            )
    })
    public ResponseEntity<Void> deleteInquiry(
            @Parameter(hidden = true) @AuthenticationPrincipal(expression = "user.storeId") UUID storeId,
            @Parameter(description = "Inquiry ID") @PathVariable UUID inquiryId
    ) {
    ...
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "201",
                    description = "문의 상담 전환 성공"
            ),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "400",
                    description = "이미 상담으로 전환된 문의 또는 잘못된 요청"
            ),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "404",
